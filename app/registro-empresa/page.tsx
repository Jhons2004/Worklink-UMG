...
export default function RegistroEmpresas() {
  ...
  const handleGoogleSuccess = async (credentialResponse: any) => {
    ...
    setError(null);
    try {
      const response = await fetch('http://localhost:8000/api/auth/google-login-company', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id_token: credentialResponse.credential }),
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.detail || 'Error en validación de Google corporativa');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/auth/login-company', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(data.detail || 'Error al iniciar sesión corporativa');
      }
